...
int comparar_restaurantes(Restaurante* r1, Restaurante* r2) {// retorno <0 se r1 deve vir antes de r2 e retorno > 0 se r1 deve vir depois de e2
    comparacoes++; // conto a comparacao da chave primaria
    
    if (r1->avaliacao < r2->avaliacao) return -1;
    if (r1->avaliacao > r2->avaliacao) return 1;

    // se for igual, comparo pelo nome
...
void quicksort(Restaurante** array, int esq, int dir) {
    ...
    // pego o pivo -> elemento do meio
    Restaurante* pivo = array[(esq + dir) / 2];

    while (i <= j) {
        while (comparar_restaurantes(array[i], pivo) < 0) {// corro o ponteiro da esquerda enquanto for menor que o pivo
            i++;
        }
        while (comparar_restaurantes(array[j], pivo) > 0) {// corro o ponteiro da direita enquanto for maior q o pivo
            j--;
        }
        
        // se eles nao se cruzaram faco a troca
        if (i <= j) {
            Restaurante* temp = array[i];
            array[i] = array[j];
            array[j] = temp;
            
            movimentacoes += 3; // conto movimentacao
    ...
    // chamada recursiva para as metades q sobraram
...
int main() {
    ...
    while (scanf("%s", idBusca) == 1) {
        if (comparar_strings(idBusca, "-1") == 0) {// leio os ids enquanto for diferente de -1
            break;
        }
    ...
    // log
    FILE* log = fopen("885492_quicksort.txt", "w");
    if (log) {
