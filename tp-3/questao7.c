...
// crio a celula q guarda o restaurante e aponta pro proximo da fila
typedef struct Celula {
    Restaurante *elemento;
    struct Celula *prox;
} Celula;

// crio o controle da fila
typedef struct {
    Celula *primeiro; // aponta pro no cabeca
    Celula *ultimo;   // aponta pro ultimo elemento inserido
} Fila;
...
// inicializo a fila com o no cabeca vazio
void iniciar_fila(Fila *fila) {
    fila->primeiro = nova_celula(NULL); 
    fila->ultimo = fila->primeiro;
}

// enfileira -> entra sempre no final da fila
void enfileirar(Fila *fila, Restaurante *r) {
    fila->ultimo->prox = nova_celula(r); // conecto no ultimo
    fila->ultimo = fila->ultimo->prox;   // ando com o ponteiro do ultimo
}

// desenfileirar
Restaurante* desenfileirar(Fila *fila) {
    if(fila->primeiro == fila->ultimo) return NULL; // vejo se ta vazia
    
    Celula *tmp = fila->primeiro->prox; // pego o primeiro da fila
    Restaurante *resp = tmp->elemento;
    
    fila->primeiro->prox = tmp->prox; // o no cabeca agora aponta pro segundo
    
    // se era o unico o ultimo volta a ser cabeca
    if(fila->ultimo == tmp) {
        fila->ultimo = fila->primeiro;
    }
...
int main() {
    ...
    if (scanf("%d", &num_operacoes) == 1) {
        for (int i = 0; i < num_operacoes; i++) {
            ...
            if (comparar_strings(comando, "I") == 0) {
                int id;
                scanf("%d", &id);
                Restaurante *r = buscar_por_id(&col, id);
                if (r != NULL) {
