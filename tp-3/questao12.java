...
class No {
    ...
    public No(Restaurante elemento) {// ponteiros esq e dir
...
class ArvoreBinaria {
    ...
    private No inserir(Restaurante x, No i) {
        if (i == null) {
            ...
            i.esq = inserir(x, i.esq); // vou p esquerda
        } else if (x.getNome().compareTo(i.elemento.getNome()) > 0) {
            i.dir = inserir(x, i.dir); // vou p direita
        } else {
            // corrigir depois se der erro
        }
        return i;
    }

    public void pesquisar(String nome) {
        System.out.print("raiz ");// comeca na raiz
        pesquisar(nome, raiz);
    }
    ...
    private void pesquisar(String nome, No i) {
        if (i == null) {
            ...
            questao12.comparacoes++; // incremento
            int cmp = nome.compareTo(i.elemento.getNome());

            if (cmp == 0) {
                System.out.println("SIM"); // achei
            } else if (cmp < 0) {
                System.out.print("esq ");
                pesquisar(nome, i.esq); // vou p esquerda
            } else {
                System.out.print("dir ");
                pesquisar(nome, i.dir);// vou p direita
            }
        }
    }

    public void caminharEmOrdem() {// em ordem -> esq raiz dir
...
public class questao12 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (sc.hasNextLine()) {// pesquiso nomes inteiros
            String nomeBusca = sc.nextLine();
            if (nomeBusca.compareTo("FIM") == 0) {// enqto for diferente de FIM
                break;
            }

            arvore.pesquisar(nomeBusca);// mando p funcao
        }

        long fimTempo = System.currentTimeMillis();// paro tempo
        long tempoTotal = fimTempo - inicioTempo;

        arvore.caminharEmOrdem();// imprimo arvore

        FileWriter writer = new FileWriter("885492_arvore_binaria.txt");
