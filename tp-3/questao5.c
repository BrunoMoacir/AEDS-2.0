...
// crio a cabeca da lista q controla tudo
typedef struct {
    Celula *primeiro;
    Celula *ultimo;
    int tamanho;
...
// funcao p criar uma celula nova na memoria
...
// inicializo a lista com o no cabeca
void iniciar_lista(Lista *lista) {
    lista->primeiro = nova_celula(NULL); // no cabeca vazio
...
void inserir_inicio(Lista *lista, Restaurante *r) {
    ...
    if(lista->primeiro == lista->ultimo) {
        lista->ultimo = tmp; // se tava vazia, ele e o ultimo 
    }
    lista->tamanho++;
}

void inserir_fim(Lista *lista, Restaurante *r) {
    lista->ultimo->prox = nova_celula(r); // conecto no ultimo
    lista->ultimo = lista->ultimo->prox;  // ando com o ponteiro do ultimo
    lista->tamanho++;
}

void inserir_posicao(Lista *lista, Restaurante *r, int pos) {
    ...
    if(pos == 0) {
        ...
        // ando ate a celula anterior a posicao que quero inserir
        Celula *i = lista->primeiro;
        for(int j = 0; j < pos; j++, i = i->prox);
        
        Celula *tmp = nova_celula(r);
        tmp->prox = i->prox; // o novo aponta pro da frente
        i->prox = tmp;       // o de tras aponta pro novo
        lista->tamanho++;
    }
}
Restaurante* remover_inicio(Lista *lista) {
    ...
    Celula *tmp = lista->primeiro->prox; // pego quem ta logo dps do no cabeca
    Restaurante *resp = tmp->elemento;
    
    lista->primeiro->prox = tmp->prox; // pulo o tmp
    
    if(lista->ultimo == tmp) {
        lista->ultimo = lista->primeiro; // se tirei o unico q tinha
    }
    
    free(tmp); // limpo da memoria
    lista->tamanho--;
...
Restaurante* remover_fim(Lista *lista) {
    ...
    lista->ultimo = i; // o penultimo vira o ultimo
...
Restaurante* remover_posicao(Lista *lista, int pos) {
    if(lista->primeiro == lista->ultimo || pos < 0 || pos >= lista->tamanho) return NULL;
    ...
    if(pos == 0) {
        ...
        // ando ate o anterior ao q quero remover
        Celula *i = lista->primeiro;
        for(int j = 0; j < pos; j++, i = i->prox);
        ...
        i->prox = tmp->prox; // costuro a lista por cima dele
...
int main() {
    ...
    Lista lista;
    iniciar_lista(&lista); // crio a lista flexivel

    char entrada[50];

    // insiro
    while (scanf("%s", entrada) == 1) {
        if (comparar_strings(entrada, "-1") == 0) {
            break;
        }
    ...
    // leio as operacoes e os comandos
    int num_operacoes;
    ...
    for (int i = 0; i < num_operacoes; i++) {
        ...
        if (comparar_strings(comando, "II") == 0) {
            int id;
            scanf("%d", &id);
            inserir_inicio(&lista, buscar_por_id(&col, id));

        } else if (comparar_strings(comando, "I*") == 0) {
            int pos, id;
            scanf("%d %d", &pos, &id);
            inserir_posicao(&lista, buscar_por_id(&col, id), pos);

        } else if (comparar_strings(comando, "IF") == 0) {
            int id;
            scanf("%d", &id);
            inserir_fim(&lista, buscar_por_id(&col, id));

        } else if (comparar_strings(comando, "RI") == 0) {
            remover_inicio(&lista);

        } else if (comparar_strings(comando, "R*") == 0) {
            int pos;
            scanf("%d", &pos);
            remover_posicao(&lista, pos);

        } else if (comparar_strings(comando, "RF") == 0) {
            remover_fim(&lista);
        }
    }

    mostrar(&lista);// mostro
