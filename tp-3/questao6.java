...
class Celula{
    public Restaurante elemento;// dado que estou guardando
    public Celula prox;// ponteiro para a celula de baixo

    public Celula(){// construtor vazio
        this(null);
    }
    
    public Celula(Restaurante elemento){// construtor com elemento
...
class Pilha{
    ...
    public void inserir(Restaurante x){// crio a celula, aponto ela para o antigo topo e ela vira o novo topo
        Celula tmp = new Celula(x);
        tmp.prox = topo;
        topo = tmp;
    }

    public Restaurante remover() throws Exception{// salvo o elemento, movo o topo para a celula de baixo e jogo a antiga fora
        if(topo == null){
        ...
        System.out.println("(R)" + resp.getNome());// modo de saida

        return resp;
    }

    public void mostrar(){// comeco no topo e vou ate null e imprimo
...
public class questao6{
    public static Restaurante buscarPorId(ColecaoRestaurantes col, int id){
        for(int i = 0; i < col.getTamanho();i++){// busca normal pelo id, ando tudo
    ...
    public static void main(String[] args)throws Exception {
        ...
        Pilha pilha = new Pilha();// inicio a pilha

        while(sc.hasNext()){// leio os ids iniciais
            ...
            if(r != null){
                pilha.inserir(r);// topo elemento vai para o topo
            }
        }

        int numOperacoes = Util.paraInt(sc.next());// leio a quantidade de operacoes

        for(int i = 0; i < numOperacoes; i++){
            String comando = sc.next();// pego o comando

            if(comando.compareTo("I") == 0){// insiro quando i
                ...
                pilha.remover();// removo quando for r
            }
        }

        pilha.mostrar();// mostro a pilha resultante
    }
}
