...
No* inserir_arvore(No *i, Restaurante *r) {
    if (i == NULL) {
        return novo_no(r); // achei espaco vazio
    } 
    
    int cmp = comparar_strings(r->nome, i->elemento->nome);
    
    if (cmp < 0) {
        i->esq = inserir_arvore(i->esq, r); // vou para a esquerda
    } else if (cmp > 0) {
        i->dir = inserir_arvore(i->dir, r); // vou para a direita
    }
    
    return i;
}

void pesquisar_arvore(No *i, char *nome) {
    if (i == NULL) {
        printf("NAO\n"); // nao achei
    } else {
        comparacoes++; // contabilizo
        ...
        if (cmp == 0) {
            ...
            printf("esq ");
            pesquisar_arvore(i->esq, nome); // procuro na esquerda
        } else {
            printf("dir ");
            pesquisar_arvore(i->dir, nome); // procuro na direita
        }
    }
}
// caminho em ordem
...
Restaurante* buscar_por_id(Colecao_Restaurantes *col, int id) {// procuro pelo id
...
int main() {
    ...
    while (scanf(" %[^\n\r]", entrada) == 1) {
        ...
        printf("raiz "); // comeco pela raiz
        pesquisar_arvore(raiz, entrada);
    }
    ...
    // arquivo log
    FILE *log = fopen("885492_arvore_binaria.txt", "w");
    if (log) {
