...
int comparar_restaurantes(Restaurante *r1, Restaurante *r2)
{
    ...
    // comparo ano
    if (r1->data_abertura.ano != r2->data_abertura.ano)
        return r1->data_abertura.ano - r2->data_abertura.ano;

    // se ano empatou comparo mes
    if (r1->data_abertura.mes != r2->data_abertura.mes)
        return r1->data_abertura.mes - r2->data_abertura.mes;

    // se mes empatou comparo dia
    if (r1->data_abertura.dia != r2->data_abertura.dia)
        return r1->data_abertura.dia - r2->data_abertura.dia;

    // se a data toda empatou desempato pelo nome
    return comparar_strings(r1->nome, r2->nome);
}

void reconstruir(Restaurante **array, int tam, int i)
{
    int maior = i; // assumo q a raiz e o maior
    int esq = 2 * i + 1; // filho esquerdo
    int dir = 2 * i + 2; // filho direito

    // vejo se o filho esquerdo e maior
    if (esq < tam &&
        comparar_restaurantes(array[esq], array[maior]) > 0)
    {
        maior = esq;
    }

    // vejo se o filho direito e maior
    if (dir < tam &&
        comparar_restaurantes(array[dir], array[maior]) > 0)
    {
        maior = dir;
    }

    // se achei alguem maior troco e continuo descendo
    if (maior != i)
    {
        ...
        movimentacoes += 3; // conto as trocas

        reconstruir(array, tam, maior);
    }
}

void construir_heap_parcial(Restaurante **array, int k)
{
    // monto o heap so p os k primeiros
    for (int i = (k / 2) - 1; i >= 0; i--)
    {
        reconstruir(array, k, i);
    }
}

void heapsort_parcial(Restaurante **array, int n, int k)
{
    construir_heap_parcial(array, k); // monto heap c k primeiros

    for (int i = k; i < n; i++)
    { // passo pelo resto do array
        comparacoes++;
        if (comparar_restaurantes(array[i], array[0]) < 0)
        { // se o de fora for menor q o topo
            
            // coloco o novo no topo
            Restaurante *tmp = array[i];
            array[i] = array[0];
            array[0] = tmp;
            movimentacoes += 3; // incremento

            reconstruir(array, k, 0); // arrumo a arvore
        }
    }

    // agora ordeno so os 10 primeiros
    for (int i = k - 1; i > 0; i--)
    {
        // troco a raiz com o ultimo 
        Restaurante *tmp = array[0];
        array[0] = array[i];
        array[i] = tmp;
        movimentacoes += 3; //incremento

        reconstruir(array, i, 0); // arrumo sem o ultimo
    }
}
...
int main()
{
    ...
    int k = 10; // dado pelo enum

    clock_t inicioTempo = clock(); // ligo cronometro

    heapsort_parcial(array, n, k); // ordeno parcial com heap

    clock_t fimTempo = clock(); // paro cronometro
    ...
    FILE *log = fopen("885492_heapsort_parcial.txt", "w"); // crio log
