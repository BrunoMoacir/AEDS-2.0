...
Restaurante* tabelaHash[TAM_TOTAL];
int reserva_livre = TAM_PRINCIPAL; // ponteiro que avanca preenchendo a area de reserva 

void inicializarTabela() {
    for (int i = 0; i < TAM_TOTAL; i++) {
        tabelaHash[i] = NULL; // limpo array
    }
}
...
int calcular_hash(char* nome) {
    ...
    for (int i = 0; nome[i] != '\0'; i++) {
        soma += (unsigned char)nome[i]; // forco unsigned p pegar o peso certo dos acentos
    }
    return soma % TAM_PRINCIPAL; 
}

void inserirHash(Restaurante* r) {
    int pos = calcular_hash(r->nome);

    if (tabelaHash[pos] == NULL) {
        tabelaHash[pos] = r; // entra na principal
    } else {
        // vou pra reserva
        if (reserva_livre < TAM_TOTAL) {
            tabelaHash[reserva_livre] = r;
            reserva_livre++; // entra na reserva
        } else {
            // reserva cheia
            printf("%s\n", r->nome); 
        }
...
void pesquisarHash(char* nome) {
    int pos = calcular_hash(nome);
    char buffer[1000];

    comparacoes++; 
    
    // olho na gaveta principal
    if (tabelaHash[pos] != NULL && comparar_strings(tabelaHash[pos]->nome, nome) == 0) {
        formatar_restaurante(tabelaHash[pos], buffer);
        printf("%d %s\n", pos, buffer); // imprimo a posicao e o formatado
        return;
    }

    // olho na reserva
    for (int i = TAM_PRINCIPAL; i < reserva_livre; i++) {
        comparacoes++;
        if (tabelaHash[i] != NULL && comparar_strings(tabelaHash[i]->nome, nome) == 0) {
            formatar_restaurante(tabelaHash[i], buffer);
            printf("%d %s\n", i, buffer); // achei na reserva
            return;
        }
    }
...
int main() {
    ...
    inicializarTabela(); // preparo o array
    ...
    while (scanf(" %[^\n\r]", entrada) == 1) { 
        ...
        pesquisarHash(entrada);
    }

    // log
    FILE *log = fopen("885492_hash_reserva.txt", "w");
    if (log != NULL) {
