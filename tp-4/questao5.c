...
typedef struct Celula {
    Restaurante *elemento; // o restaurante
    struct Celula *prox;   // ponteiro q aponta p o prox
} Celula;

Celula* tabelaHash[TAM_TAB];

void inicializarTabela() {
    for (int i = 0; i < TAM_TAB; i++) {
        tabelaHash[i] = NULL; // inicializo as gavetas
    }
}
...
void inserirHash(Restaurante* r) {
    ...
    nova->elemento = r;
    nova->prox = NULL; // vai pro final da fila entao o prox e null

    if (tabelaHash[pos] == NULL) {
        // se a gaveta ta vazia, ele vira o primeiro da fila
        tabelaHash[pos] = nova;
    } else {
        // deu colisao
        Celula* i = tabelaHash[pos];
        
        // vou pulando de vagao em vagao ate achar o ultimo
        while (i->prox != NULL) {
            i = i->prox;
        }
        
        // novo restaurante vira o final
        i->prox = nova;
    }
}
...
void pesquisarHash(char* nome) {
    ...
    // pego a cabeca da fila que mora na gaveta calculada
    Celula* i = tabelaHash[pos];

    // varro a fila ate ela acabar
    while (i != NULL) {
        comparacoes++; 
        
        if (comparar_strings(i->elemento->nome, nome) == 0) {
            formatar_restaurante(i->elemento, buffer);
            printf("%d %s\n", pos, buffer); // achei e imprimo
            return;
        }
        
        i = i->prox; // pulo pro proximo vagao da fila
    }
...
int main() {
    ...
    // log
    FILE *log = fopen("885492_hash_indireta.txt", "w");
    if (log != NULL) {
