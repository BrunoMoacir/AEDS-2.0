...
typedef struct Celula {
    char letra;             // a letra que essa gaveta representa
    NoTrie *filhoTrie;      // o ponteiro para o no da Trie dessa letra
    struct Celula *prox;    // ponteiro pro prox irmao da lista
} Celula;

struct NoTrie {
    char elemento;          // letra atual
    bool folha;             // fim da palavra
    Restaurante *restaurante; 
    Celula *primeiroFilho;  // primeiro da lista
};

NoTrie* raiz = NULL;

NoTrie* novoNoTrie(char c) {
    NoTrie *novo = (NoTrie*)malloc(sizeof(NoTrie));
    novo->elemento = c;
    novo->folha = false;
    novo->restaurante = NULL;
    novo->primeiroFilho = NULL; // a lista de filhos nasce vazia
    return novo;
}

NoTrie* inserirOuObterFilho(NoTrie *pai, char c) {
    Celula *i = pai->primeiroFilho;
    
    // varro a lista
    while (i != NULL) {
        if (i->letra == c) {
            return i->filhoTrie; // achei e retorno
        }
        i = i->prox;
    }
    
    // se nao achei crio um novo
    Celula *nova = (Celula*)malloc(sizeof(Celula));
    nova->letra = c;
    nova->filhoTrie = novoNoTrie(c); // crio o no real da Trie
    
    nova->prox = pai->primeiroFilho;
    pai->primeiroFilho = nova;
    
    return nova->filhoTrie;
}

void inserir(Restaurante *r) {
    if (raiz == NULL) raiz = novoNoTrie(' '); // inicializo a raiz vazia
    ...
    for (int i = 0; nome[i] != '\0'; i++) {
        atual = inserirOuObterFilho(atual, nome[i]);
    }

    // terminei de varrer o nome garanto a folha e guardo o restaurante
    atual->folha = true;
    atual->restaurante = r;
}
...
void pesquisar(char *nome) {
    ...
    for (int i = 0; nome[i] != '\0'; i++) {
        char c = nome[i];
        Celula *cel = atual->primeiroFilho; // pego a lista de filhos
        bool achou = false;

        // varro a lista encadeada desse no pra procurar a proxima letra
        while (cel != NULL) {
            comparacoes++; 
            if (cel->letra == c) {
                achou = true;
                atual = cel->filhoTrie; // vou pro prox nivel
                break;
            }
            cel = cel->prox;
        }
    ...
    if (atual->folha) {
        ...
        printf("SIM %s\n", buffer); // achei e imprimo o restaurante formatado
    } else {
        printf("NAO\n"); // palavra incompleta
    }
}
...
int main() {
    ...
    while (scanf(" %[^\n\r]", entrada) == 1) { 
        ...
        int fim = tamanho_string(entrada) - 1;
        while (fim >= 0 && (entrada[fim] == '\r' || entrada[fim] == ' ' || entrada[fim] == '\n')) {
            entrada[fim] = '\0';
            fim--;
        }
    ...
    // log
    FILE *log = fopen("885492_arvore_trie_lista.txt", "w");
    if (log != NULL) {
